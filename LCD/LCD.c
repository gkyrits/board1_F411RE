...
void LCD_DisplayImage(POINT Xstart, POINT Ystart, sIMAGE* Image){
	POINT xp,yp;
	...
	for(int y=0; y<Image->Height; y++){
		for(int x=0; x<Image->Width; x++){
			idx=(y*Image->Width) + x;
			col_idx = Image->image_table[idx]*4;
			red   = Image->color_table[col_idx+2] >>3;
			green = Image->color_table[col_idx+1] >>2;
			blue  = Image->color_table[col_idx]   >>3;
			red   = red   << 11;
			green = green << 5;
			color = red | green | blue;
			xp = x;
			yp = y;
			LCD_SetPointlColor(xp,yp,color);
		}
	}
}

void LCD_DisplayImageLine(POINT Xstart, POINT Ystart, char *buff, uint16_t width){
	POINT xp,yp;
	uint16_t red,green,blue,color;
	int col_idx;
	...
	for(int x=0; x<width; x++){
		col_idx = x*3;
		red   = buff[col_idx]   >>3;
		green = buff[col_idx+1] >>2;
		blue  = buff[col_idx+2] >>3;
		red   = red   << 11;
		green = green << 5;
		color = red | green | blue;
		xp = x;
